                ...
                {isClient && user ? (
                    <span className="text-gray-800 font-semibold">
                        {user?.isBuyer && user?.isSeller
                            ? `Buyer and Seller - Connected: ${user?.address.slice(
                                  0,
                                  6
                              )}...${user?.address.slice(-4)}`
                            : user?.isBuyer
                            ? `Buyer - Connected: ${user?.address.slice(
                                  0,
                                  6
                              )}...${user?.address.slice(-4)}`
                            : user?.isSeller
                            ? `Seller - Connected: ${user?.address.slice(
                                  0,
                                  6
                              )}...${user?.address.slice(-4)}`
                            : `Not a Buyer or Seller - Connected: ${user?.address.slice(
                                  0,
                                  6
                              )}...${user?.address.slice(-4)}`}
                    </span>
